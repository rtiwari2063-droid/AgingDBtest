...
# ==========================================
# 2. PRE-LOAD HEAVY DATA & FOLDER PATHS
# ==========================================
# GITHUB FIX: All paths are now RELATIVE to the app directory.
# NO absolute paths (like ~/Desktop/) are allowed in Shinylive.

# ---------------------------------------------------------
# A. LOAD OPTIMIZED UNIVERSE & DICTIONARIES
# ---------------------------------------------------------
print("Setting up Background Universe...")
path_universe <- "optimized_universe.rds" 
if(!file.exists(path_universe)) stop("CRITICAL: optimized_universe.rds not found.")

universe_df <- readRDS(path_universe)
universe_genes <- unique(universe_df$ENTREZID)
N_total_db_genes <- length(universe_genes)

# Create Lightning-Fast Lookup Dictionaries
...
# ---------------------------------------------------------
# B. LOAD THE SINGLE MASTER DATABASE (WIDE FORMAT)
# ---------------------------------------------------------
print("Loading Master Database...")
path_master <- "Master_Hallmark_Wide.rds" 
if(!file.exists(path_master)) stop("CRITICAL: Master_Hallmark_Wide.rds not found.")

master_db <- readRDS(path_master)

unique_hallmarks <- sort(unique(as.character(master_db$HallMarks)))

combined_exo_genes <- master_db %>% 
  dplyr::filter(In_Exosome == "Yes") %>% 
  dplyr::pull(Gene_ID) %>% 
  unique()
...
# C. TISSUE MAPPING
# ---------------------------------------------------------
path_tissue_mapping <- "rna_single_cell_clusters.tsv"

if (file.exists(path_tissue_mapping)) {
  tissue_mapping_df <- read.delim(path_tissue_mapping, stringsAsFactors = FALSE)
  if("Cell.type.class" %in% colnames(tissue_mapping_df)) {
    colnames(tissue_mapping_df)[colnames(tissue_mapping_df) == "Cell.type.class"] <- "Cell_Type_Class"
  }
} else {
...
# ---------------------------------------------------------
# D. LOAD OPTIMIZED NETWORKS
# ---------------------------------------------------------
print("Loading Optimized Network Data...")

path_tf_network <- "optimized_tf_network.rds" 
if(file.exists(path_tf_network)) {
  db_tf <- readRDS(path_tf_network)
} else {
  db_tf <- data.frame(source = character(), target = character(), is_stimulation = character(), is_inhibition = character(), stringsAsFactors=FALSE)
}

path_kinase_network <- "optimized_kinase_network.rds" 
if(file.exists(path_kinase_network)) {
  db_kinase <- readRDS(path_kinase_network)
} else {
  db_kinase <- data.frame(source = character(), target = character(), stringsAsFactors=FALSE)
}
...
custom_css <- tags$head(tags$style(HTML("
  ...
  .hallmark-box:hover {
    ...
    box-shadow: 0 20px 25px -5px rgba(59, 130, 246, 0.4), 0 8px 10px -6px rgba(59, 130, 246, 0.1);
    background: linear-gradient(145deg, #1e40af, #60a5fa);
    color: white;
  }
  
  .hallmark-custom-icon {
    width: 64px;
    height: 64px;
    margin-bottom: 18px;
    object-fit: contain;
  ...
  .hallmark-box:hover .hallmark-custom-icon {
...
ui <- page_navbar(
  ...
  nav_panel(title = "Browse Hallmarks", value = "tab_home",
            conditionalPanel(
              condition = "output.home_page === 'grid'",
              div(class="container text-center mt-5 mb-5", style = "max-width: 1400px;",
                  h2("Browse Genes by Aging Hallmark", class="fw-bold mb-5"),
                  uiOutput("hallmark_grid_ui")
              )
            ),
            conditionalPanel(
              condition = "output.home_page === 'result'",
              div(class="container mt-4 mb-5", style = "max-width: 1000px;",
                  actionButton("back_home_btn", "← Back to Grid", class="btn btn-outline-secondary fw-bold mb-4"),
                  
                  card(class = "shadow-sm border-0 mb-4", card_body(
                    h4("Filter Selection", class="text-primary fw-bold mb-3"),
                    layout_columns(
                      col_widths = c(4, 4, 4),
                      selectInput("home_db_choice", label = "Gene Set Database:", width = "100%", choices = list("Extended" = "Extended", "Consensus" = "Consensus")),
                      radioButtons("home_ev_choice", "Evidence Filter:", inline = TRUE, choices = list("With Evidence" = "evidence", "No Evidence" = "no_evidence")),
                      radioButtons("home_spec_level", "Specificity Level:", inline = TRUE, choices = list("Whole Body" = "all", "By Tissue" = "tissue", "By Cell Type Class" = "cell"))
                    ),
                    conditionalPanel("input.home_spec_level == 'tissue'", selectizeInput("home_tissue_choice", "Select Tissue(s):", width = "100%", choices = available_tissues, multiple = TRUE), uiOutput("home_tissue_cell_ui")),
                    conditionalPanel("input.home_spec_level == 'cell'", selectizeInput("home_cell_choice", "Select Cell Type(s):", width = "100%", choices = cell_class_choices, multiple = TRUE))
                  )),
                  
                  card(class = "shadow-sm border-0", 
                       card_header(class = "bg-transparent", h3(textOutput("selected_hallmark_title"), class="text-primary fw-bold m-0")),
                       card_body(uiOutput("home_error_msg"), DTOutput("hallmark_genes_table"))
                  )
              )
            )
  ),
  
  # --- TAB 1: HALLMARK ENRICHMENT ---
  nav_panel(title = "Hallmark Enrichment", value = "tab_enrich_inputs",
            conditionalPanel(
              condition = "output.enrich_page === 'input'",
              div(class = "container mt-5 mb-5", style = "max-width: 900px;",
                  card(class = "shadow-sm border-0", card_body(
                    h3("Hallmarks Enrichment", class = "text-primary fw-bold"),
                    p("Identify significantly overrepresented aging hallmarks in your input gene set.", class="text-muted"),
                    hr(),
                    h4(tags$span(class = "badge bg-primary rounded-circle", "1"), " Enter gene list (Entrez IDs)", class = "fw-bold mt-2"),
                    div(class = "mb-2", actionButton("sample_btn", "Load sample genes", class = "btn btn-sm btn-info text-white fw-bold"), actionButton("reset_btn", "Reset list", class = "btn btn-outline-secondary btn-sm ms-2")),
                    textAreaInput("gene_input", label = NULL, height = "150px", placeholder = "e.g. 7007\n9370\n652"),
                    h4(tags$span(class = "badge bg-primary rounded-circle", "2"), " Select parameters", class = "fw-bold mt-4"),
                    layout_columns(
                      col_widths = c(6, 6),
                      selectInput("db_choice", label = "Gene Set Database:", width = "100%", choices = list("Extended" = "Extended", "Consensus" = "Consensus")),
                      radioButtons("enrich_ev_choice", "Evidence Filter:", inline = TRUE, choices = list("With Evidence" = "evidence", "No Evidence" = "no_evidence"))
                    ),
                    radioButtons("enrich_spec_level", "Specificity Level:", inline = TRUE, choices = list("Whole Body" = "all", "By Tissue" = "tissue", "By Cell Type Class" = "cell")),
                    conditionalPanel("input.enrich_spec_level == 'tissue'", selectizeInput("enrich_tissue_choice", "Select Tissue(s):", width = "100%", choices = available_tissues, multiple = TRUE), uiOutput("enrich_tissue_cell_ui")),
                    conditionalPanel("input.enrich_spec_level == 'cell'", selectizeInput("enrich_cell_choice", "Select Cell Type(s):", width = "100%", choices = cell_class_choices, multiple = TRUE)),
                    h4(tags$span(class = "badge bg-primary rounded-circle", "3"), " Submit", class = "fw-bold mt-4"),
                    uiOutput("enrich_error_msg"), actionButton("run_btn", "Run analysis →", class = "btn btn-lg btn-primary w-100 fw-bold mt-2")
                  ))
              )
            ),
            conditionalPanel(
              condition = "output.enrich_page === 'result'",
              div(class = "container mt-4 mb-5", style = "max-width: 1200px;",
                  layout_columns(col_widths = c(6, 6), actionButton("back_enrich_btn", "← Back to Inputs", class = "btn btn-outline-secondary fw-bold"), div(style = "text-align: right;", downloadButton("dl_enrich_csv", "📥 Download Table", class = "btn btn-success fw-bold"))),
                  h3("Analysis Results", class = "fw-bold mt-4 mb-3 text-primary border-bottom border-primary pb-2"),
                  uiOutput("mapping_summary_ui"),
                  div(class = "filter-bar", 
                      checkboxInput("enable_filters", tags$strong("Enable Significance Filters (FDR & Minimum Overlap)"), value = FALSE),
                      conditionalPanel(condition = "input.enable_filters == true", layout_columns(col_widths = c(6, 6), sliderInput("fdr_cutoff", "FDR Threshold:", min = 0.01, max = 0.25, value = 0.05, step = 0.01, width = "100%"), sliderInput("min_overlap", "Minimum Gene Overlap:", min = 1, max = 20, value = 1, step = 1, width = "100%")))
                  ),
                  uiOutput("kpi_boxes_enrichment"), br(), card(class = "shadow-sm border-0 mb-4", card_body(DTOutput("enrichmentTable"))),
                  card(class = "shadow-sm border-0", card_header(class = "d-flex justify-content-between align-items-center bg-transparent", radioButtons("plot_type", label = "Visualization:", inline = TRUE, choices = list("Bar plot" = "bar", "Bubble plot" = "bubble", "Circular plot" = "circular")), downloadButton("dl_enrich_plot", "🖼️ Download Image", class = "btn btn-outline-primary fw-bold")), div(class = "plot-container", plotOutput("enrichmentPlot", height = "650px")))
              )
            )
  ),
  
  # --- TAB 2: GENE MAPPING DROPDOWN ---
  nav_menu(
    title = "Gene Mapping",
    nav_panel(title = "Map to Hallmarks", value = "tab_map_inputs",
              conditionalPanel(
                condition = "output.map_page === 'input'",
                div(class = "container mt-5 mb-5", style = "max-width: 900px;",
                    card(class = "shadow-sm border-0", card_body(
                      h3("Map Gene Sets to Hallmarks", class = "text-primary fw-bold"), hr(),
                      layout_columns(col_widths = c(6, 6), selectInput("mapping_db_choice", label = "1. Select Database:", width = "100%", choices = list("Extended" = "Extended", "Consensus" = "Consensus")), radioButtons("map_ev_choice", "Evidence Filter:", inline = TRUE, choices = list("With Evidence" = "evidence", "No Evidence" = "no_evidence"))),
                      radioButtons("map_spec_level", "2. Specificity Level:", inline = TRUE, choices = list("Whole Body" = "all", "By Tissue" = "tissue", "By Cell Type Class" = "cell")),
                      conditionalPanel("input.map_spec_level == 'tissue'", selectizeInput("map_tissue_choice", "Select Tissue(s):", width = "100%", choices = available_tissues, multiple = TRUE), uiOutput("map_tissue_cell_ui")),
                      conditionalPanel("input.map_spec_level == 'cell'", selectizeInput("map_cell_choice", "Select Cell Type(s):", width = "100%", choices = cell_class_choices, multiple = TRUE)),
                      div(class="d-flex justify-content-between align-items-end mt-3", tags$label("3. Paste Gene IDs:", class="fw-bold mb-0"), div(actionButton("sample_map_btn", "Load sample", class = "btn btn-sm btn-info text-white"), actionButton("reset_map_btn", "Reset", class = "btn btn-outline-secondary btn-sm ms-2"))),
                      textAreaInput("mapping_input", label = NULL, height = "220px", placeholder = "Enter Entrez IDs..."),
                      uiOutput("mapping_error_msg"), actionButton("run_mapping_btn", "Map to Hallmarks →", class = "btn btn-lg btn-primary w-100 fw-bold mt-3")
                    ))
                )
              ),
              conditionalPanel(
                condition = "output.map_page === 'result'",
                div(class = "container mt-4 mb-5", style = "max-width: 1000px;",
                    layout_columns(col_widths = c(6, 6), actionButton("back_map_btn", "← Back to Inputs", class = "btn btn-outline-secondary fw-bold"), div(style = "text-align: right;", downloadButton("dl_map_csv", "📥 Download Mapping", class = "btn btn-success fw-bold"))),
                    h3("Gene Mapping Results", class = "fw-bold mt-4 mb-3 text-primary border-bottom border-primary pb-2"),
                    uiOutput("kpi_boxes_mapping"), br(), card(class = "shadow-sm border-0", card_body(DTOutput("mappingTable")))
                )
              )
    ),
    nav_panel(title = "Exosome Presence Check", value = "tab_exo_inputs",
              conditionalPanel(
                condition = "output.exo_page === 'input'",
                div(class = "container mt-5 mb-5", style = "max-width: 900px;",
                    card(class = "shadow-sm border-0", card_body(
                      h3("Exosome Presence Check", class = "text-primary fw-bold"), 
                      p("Check if your genes are present in exosome datasets. Genes not found in the selected background aging hallmarks database will be automatically filtered out.", class="text-muted"), hr(),
                      layout_columns(col_widths = c(6, 6), selectInput("exo_db_choice", label = "1. Select Hallmark Database:", width = "100%", choices = list("Extended" = "Extended", "Consensus" = "Consensus")), radioButtons("exo_ev_choice", "Evidence Filter:", inline = TRUE, choices = list("With Evidence" = "evidence", "No Evidence" = "no_evidence"))),
                      radioButtons("exo_spec_level", "2. Specificity Level:", inline = TRUE, choices = list("Whole Body" = "all", "By Tissue" = "tissue", "By Cell Type Class" = "cell")),
                      conditionalPanel("input.exo_spec_level == 'tissue'", selectizeInput("exo_tissue_choice", "Select Tissue(s):", width = "100%", choices = available_tissues, multiple = TRUE), uiOutput("exo_tissue_cell_ui")),
                      conditionalPanel("input.exo_spec_level == 'cell'", selectizeInput("exo_cell_choice", "Select Cell Type(s):", width = "100%", choices = cell_class_choices, multiple = TRUE)),
                      div(class="d-flex justify-content-between align-items-end mt-4", tags$label("3. Paste Gene IDs (Entrez):", class="fw-bold mb-0"), div(actionButton("sample_exo_btn", "Load sample", class = "btn btn-sm btn-info text-white"), actionButton("reset_exo_btn", "Reset", class = "btn btn-outline-secondary btn-sm ms-2"))),
                      textAreaInput("exo_input", label = NULL, height = "180px", placeholder = "Enter Entrez IDs..."),
                      uiOutput("exo_error_msg"), actionButton("run_exo_btn", "Check Exosomes →", class = "btn btn-lg btn-primary w-100 fw-bold mt-3")
                    ))
                )
              ),
              conditionalPanel(
                condition = "output.exo_page === 'result'",
                div(class = "container mt-4 mb-5", style = "max-width: 1000px;",
                    layout_columns(col_widths = c(6, 6), actionButton("back_exo_btn", "← Back to Inputs", class = "btn btn-outline-secondary fw-bold"), div(style = "text-align: right;", downloadButton("dl_exo_csv", "📥 Download Table", class = "btn btn-success fw-bold"))),
                    h3("Exosome Presence Results", class = "fw-bold mt-4 mb-3 text-primary border-bottom border-primary pb-2"), uiOutput("exo_summary_ui"), br(), card(class = "shadow-sm border-0", card_body(DTOutput("exoTable")))
                )
              )
    )
  ),
  
  # --- TAB 3 & 4 (NETWORK ANALYSIS DROPDOWN) ---
  nav_menu(
    title = "Network Analysis",
    nav_panel(title = "TF-Target Interactions", value = "tab_tf_net",
              div(class = "container mt-5 mb-5", style = "max-width: 1200px;",
                  card(class = "shadow-sm border-0", card_body(
                    h3("Transcription Regulatory Network", class = "text-primary fw-bold"), p("Paste your gene list. The tool will map internal regulatory interactions strictly BETWEEN the genes you provide.", class="text-muted"), hr(),
                    layout_columns(col_widths = c(4, 4, 4), div(tags$label("1. Select Database:"), selectInput("tf_db_choice", label = NULL, width = "100%", choices = list("Extended" = "Extended", "Consensus" = "Consensus"))), div(tags$label("Evidence Filter:"), radioButtons("tf_ev_choice", label = NULL, inline = TRUE, choices = list("With Evidence" = "evidence", "No Evidence" = "no_evidence"))), div(tags$label("2. Specificity Level:"), radioButtons("tf_spec_level", label = NULL, inline = TRUE, choices = list("Whole Body" = "all", "By Tissue" = "tissue", "By Cell Type Class" = "cell")))),
                    conditionalPanel("input.tf_spec_level == 'tissue'", selectizeInput("tf_tissue_choice", "Select Tissue(s):", width = "100%", choices = available_tissues, multiple = TRUE), uiOutput("tf_tissue_cell_ui")),
                    conditionalPanel("input.tf_spec_level == 'cell'", selectizeInput("tf_cell_choice", "Select Cell Type(s):", width = "100%", choices = cell_class_choices, multiple = TRUE)),
                    div(class="d-flex justify-content-between align-items-end mt-4", tags$label("3. Paste Gene IDs (Entrez IDs):", class="fw-bold mb-0 text-primary"), div(actionButton("sample_tf_btn", "Load sample", class = "btn btn-sm btn-info text-white fw-bold"), actionButton("reset_tf_btn", "Reset", class = "btn btn-outline-secondary btn-sm ms-2"))),
                    textAreaInput("tf_input", label = NULL, height = "150px", placeholder = "e.g. 7124\n2\n1029\n5970\n2099\n4609"), uiOutput("tf_status_msg"), actionButton("run_tf_btn", "Map Internal TF Network →", class = "btn btn-lg btn-primary w-100 fw-bold mt-3")
                  )), br(), uiOutput("tf_results_panel")
              )
    ),
    nav_panel(title = "Kinase-Substrate Interactions", value = "tab_kinase_net",
              div(class = "container mt-5 mb-5", style = "max-width: 1200px;",
                  card(class = "shadow-sm border-0", card_body(
                    h3("Substrate Kinase Network", class = "text-primary fw-bold"), p("Paste your gene list. The tool will map internal phosphorylation interactions strictly BETWEEN the genes you provide.", class="text-muted"), hr(),
                    layout_columns(col_widths = c(4, 4, 4), div(tags$label("1. Select Database:"), selectInput("kin_db_choice", label = NULL, width = "100%", choices = list("Extended" = "Extended", "Consensus" = "Consensus"))), div(tags$label("Evidence Filter:"), radioButtons("kin_ev_choice", label = NULL, inline = TRUE, choices = list("With Evidence" = "evidence", "No Evidence" = "no_evidence"))), div(tags$label("2. Specificity Level:"), radioButtons("kin_spec_level", label = NULL, inline = TRUE, choices = list("Whole Body" = "all", "By Tissue" = "tissue", "By Cell Type Class" = "cell")))),
                    conditionalPanel("input.kin_spec_level == 'tissue'", selectizeInput("kin_tissue_choice", "Select Tissue(s):", width = "100%", choices = available_tissues, multiple = TRUE), uiOutput("kin_tissue_cell_ui")),
                    conditionalPanel("input.kin_spec_level == 'cell'", selectizeInput("kin_cell_choice", "Select Cell Type(s):", width = "100%", choices = cell_class_choices, multiple = TRUE)),
                    div(class="d-flex justify-content-between align-items-end mt-4", tags$label("3. Paste Gene IDs (Entrez IDs):", class="fw-bold mb-0 text-primary"), div(actionButton("sample_kin_btn", "Load sample", class = "btn btn-sm btn-info text-white fw-bold"), actionButton("reset_kin_btn", "Reset", class = "btn btn-outline-secondary btn-sm ms-2"))),
                    textAreaInput("kin_input", label = NULL, height = "150px", placeholder = "e.g. 5594\n5595\n4193\n836\n207\n3551"), uiOutput("kin_status_msg"), actionButton("run_kin_btn", "Map Internal Kinase Network →", class = "btn btn-lg btn-primary w-100 fw-bold mt-3")
                  )), br(), uiOutput("kin_results_panel")
              )
    )
  ),
...
server <- function(input, output, session) {
  ...
  output$hallmark_genes_table <- renderDT({
    ...
    # NEW FIX: Generate HTML Hyperlinks to NCBI
    hm_genes <- hm_genes %>% 
      dplyr::mutate(
    ...
    datatable(hm_genes, class = 'table table-striped table-hover', escape = FALSE, options = list(pageLength = 15), rownames = FALSE) %>% 
      formatStyle('Symbol', color = '#0EA5E9', fontWeight = 'bold')
  })
  ...
  output$tf_results_panel <- renderUI({
    ...
    # GITHUB FIX: Unknown Legend key applied here
  ...
  output$tfNetwork <- renderVisNetwork({
    ...
    vis_edges_base <- data.frame(
      from = edges$source, to = edges$target, type = edges$type,
      is_stimulation = edges$is_stimulation, is_inhibition = edges$is_inhibition,
    ...
    # GITHUB FIX: Handle the "Both" scenario by duplicating the edge row to draw two lines
    ...
    final_edges$color <- dplyr::case_when(
      final_edges$type == "hallmark_association" ~ "#CBD5E1",
      final_edges$stim & !final_edges$inhib ~ "#10B981",
      !final_edges$stim & final_edges$inhib ~ "#EF4444",
      TRUE ~ "#9CA3AF" # The Unknown Default
    )
    
    final_edges$arrows <- dplyr::case_when(
      final_edges$type == "hallmark_association" ~ "",
      !final_edges$stim & final_edges$inhib ~ "",
      TRUE ~ "to"
    )
    
    # Enable smooth curves so duplicate edges separate visually
    final_edges$smooth <- TRUE
    
    visNetwork(nodes, final_edges) %>% 
      visGroups(groupname = "Aging Hallmark", color = list(background = "#FBBF24", border = "#D97706")) %>% 
      visGroups(groupname = "Transcription Factor", color = list(background = "#EF4444", border = "#B91C1C")) %>% 
      visGroups(groupname = "TF & Target", color = list(background = "#9333EA", border = "#6D28D9")) %>% 
      visGroups(groupname = "Target Gene", color = list(background = "#0EA5E9", border = "#0284C7")) %>% 
      visExport(type = "png", name = "TF_Network", label = "Export PNG", style = "background-color: #0EA5E9; color: white; border: none; padding: 5px 10px; border-radius: 4px; cursor: pointer; float: right; margin-top: 10px;") %>% 
      ...
      visPhysics(solver = "forceAtlas2Based", forceAtlas2Based = list(gravitationalConstant = -100, springLength = 200), stabilization = list(enabled = TRUE, iterations = 150)) %>%
      visEvents(type = "once", stabilizationIterationsDone = "function() { this.setOptions({ physics: false }); }") %>% visInteraction(dragNodes = TRUE)
  })
  ...
  output$kinNetwork <- renderVisNetwork({
    ...
    visNetwork(nodes, vis_edges) %>% 
      visGroups(groupname = "Aging Hallmark", color = list(background = "#FBBF24", border = "#D97706")) %>% 
      visGroups(groupname = "Kinase", color = list(background = "#8B5CF6", border = "#6D28D9")) %>% 
      visGroups(groupname = "Kinase & Substrate", color = list(background = "#EC4899", border = "#A855F7")) %>% 
      visGroups(groupname = "Substrate", color = list(background = "#10B981", border = "#047857")) %>% 
      visExport(type = "png", name = "Kinase_Network", label = "Export PNG", style = "background-color: #0EA5E9; color: white; border: none; padding: 5px 10px; border-radius: 4px; cursor: pointer; float: right; margin-top: 10px;") %>% 
      visOptions(highlightNearest = list(enabled = TRUE, degree = 1, hover = TRUE), nodesIdSelection = TRUE) %>% 
      visEdges(smooth = FALSE) %>% 
      visPhysics(solver = "forceAtlas2Based", forceAtlas2Based = list(gravitationalConstant = -100, springLength = 200), stabilization = list(enabled = TRUE, iterations = 150)) %>% visEvents(type = "once", stabilizationIterationsDone = "function() { this.setOptions({ physics: false }); }") %>% visInteraction(dragNodes = TRUE)
  })
